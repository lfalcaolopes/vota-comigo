...
import type { ProposicaoWithVotacoes, RankedProposicao } from '../types/proposicoes.types';
import {
...
describe('compareDataApresentacao', () => {
  describe('when items have different dataApresentacao', () => {
    it('sorts later dates first (descending)', () => {
      // Arrange
      const older = ranked({ externalIdProposicao: 1, dataApresentacao: '2022-01-01T00:00:00Z' });
      const newer = ranked({ externalIdProposicao: 2, dataApresentacao: '2024-01-01T00:00:00Z' });

      // Act
      const sorted = [older, newer].sort(compareDataApresentacao);

      // Assert
      expect(sorted.map((r) => r.proposicao.externalIdProposicao)).toEqual([2, 1]);
    });
  });
  ...
  describe('when one item has no dataApresentacao', () => {
    it('puts items without a date at the end', () => {
      // Arrange
      const withDate = ranked({ externalIdProposicao: 1, dataApresentacao: '2024-01-01T00:00:00Z' });
      const nullDate = ranked({ externalIdProposicao: 2, dataApresentacao: null });

      // Act
      const sorted = [nullDate, withDate].sort(compareDataApresentacao);

      // Assert
      expect(sorted.map((r) => r.proposicao.externalIdProposicao)).toEqual([1, 2]);
    });

    it('puts two null-date items after all dated items, keeping them stable via tie-break', () => {
      // Arrange
      const withDate = ranked({ externalIdProposicao: 1, dataApresentacao: '2023-06-15T00:00:00Z' });
      const nullA = ranked({ externalIdProposicao: 2, dataApresentacao: null, ano: 2023, numero: 10 });
      const nullB = ranked({ externalIdProposicao: 3, dataApresentacao: null, ano: 2024, numero: 5 });
      ...
      expect(sorted.map((r) => r.proposicao.externalIdProposicao)).toEqual([1, 3, 2]);
    });
  });
  ...
  describe('when items share the same dataApresentacao', () => {
    it('breaks ties by ano desc, numero desc, siglaTipo asc, externalIdProposicao asc', () => {
      ...
      const items = [
        ranked({ ...base, externalIdProposicao: 10, ano: 2023, numero: 5, siglaTipo: 'PL' }),
        ranked({ ...base, externalIdProposicao: 11, ano: 2024, numero: 5, siglaTipo: 'PL' }),
        ranked({ ...base, externalIdProposicao: 12, ano: 2024, numero: 9, siglaTipo: 'PL' }),
      ];
      ...
      // Assert
      expect(sorted.map((r) => r.proposicao.externalIdProposicao)).toEqual([12, 11, 10]);
    });
  });
});
