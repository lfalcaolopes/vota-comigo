import { createOpenrouterResumoIaClient } from '../openrouter-resumo-ia-client';
...
describe('createOpenrouterResumoIaClient', () => {
  describe('when the API returns a valid generated response', () => {
    it('returns ok:true with the parsed response', async () => {
      // Arrange
      const modelResponse = { status: 'generated', resumoCard: 'Card.', resumoDetalhe: 'Detalhe.' };
      const fetch = makeFetch({ ok: true, body: openrouterBody(modelResponse) });
      const client = createOpenrouterResumoIaClient({ apiKey: 'key', model: 'model-x', fetch });
    ...
    it('calls the correct OpenRouter URL', async () => {
      ...
      const fetch = makeFetch({
        ok: true,
        body: openrouterBody({ status: 'generated', resumoCard: 'R.', resumoDetalhe: 'D.' }),
      });
      const client = createOpenrouterResumoIaClient({ apiKey: 'key', model: 'model-x', fetch });
    ...
    it('sends Authorization header with the API key', async () => {
      ...
      const client = createOpenrouterResumoIaClient({ apiKey: 'my-api-key', model: 'model-x', fetch });
    ...
    it('sends the model and response_format in the request body', async () => {
      ...
      const client = createOpenrouterResumoIaClient({ apiKey: 'key', model: 'gpt-4o', fetch });
  ...
  describe('when the API returns an insufficient_source response', () => {
    it('returns ok:true with status insufficient_source', async () => {
      // Arrange
      const modelResponse = { status: 'insufficient_source', resumoCard: null, resumoDetalhe: null };
      const fetch = makeFetch({ ok: true, body: openrouterBody(modelResponse) });
      const client = createOpenrouterResumoIaClient({ apiKey: 'key', model: 'model-x', fetch });
  ...
  describe('when the model response content is invalid', () => {
    ...
    it('returns ok:false when parsed content fails schema validation', async () => {
      // Arrange
      const invalidContent = { status: 'error', resumoCard: 'x', resumoDetalhe: 'x' };
      const fetch = makeFetch({ ok: true, body: openrouterBody(invalidContent) });
      const client = createOpenrouterResumoIaClient({ apiKey: 'key', model: 'model-x', fetch });
